...
class FrameDrawer():
    ...
    def initialize_drawer(self):
        ...
        drawer_layout = {
            ...
            'rigid_flow_diff':  ([int(hu*3), int(wu*3)], [int(hu*4), int(wu*4)]),
            'opt_flow_diff':    ([int(hu*3), int(wu*3)], [int(hu*4), int(wu*4)]),

            # (Experiement Ver. only)
            'warp_diff':        ([int(hu*3), int(wu*2)], [int(hu*4), int(wu*3)]),
        }
