...
def rotate3d(vec, degrees,rotation_axis = np.array([0, 0, 1])):


    rotation_radians = degrees*3.1415926535/180.0
    ...
    return rotated_vec
# wingman loss
def compute_wingman_loss(poseleft, poseright):
    """ Function to compute Wingman Loss: 
    Args:
        pose1: poses vector 1
        pose2: poses vector 2
    Return:
        wing_loss: euclide distance between 2 poses
    """
    wing_loss = 0
    R1 = poseleft[3:6]
    R2 = poseright[3:6]
    T1 = poseleft[:3]
    T2 = rotate3d(poseright[:3],60)
    for pose, pose_inv in zip(poseleft, poseright):
        wing_loss += torch.cdist(pose, pose_inv, p=2).abs().clamp(0, 1).sum()
    return wing_loss
...
def compute_pairwase_loss2(tgt_img, ref_img, tgt_depth, ref_depth, pose, pose_inv, intrinsic, with_auto_mask, padding_mode):

    ref_img_warped, valid_mask, projected_depth, computed_depth = inverse_warp2(ref_img, tgt_depth, ref_depth, pose, intrinsic, padding_mode)

    diff_distance_pose = torch.cdist(pose, pose_inv, p=2).abs().clamp(0, 1)

    # compute all loss
    wing_loss = mean_on_mask(diff_distance_pose, valid_mask)
