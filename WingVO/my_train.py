...
from loss_functions import compute_smooth_loss, compute_photo_and_geometry_loss, compute_errors, compute_wingman_loss
...
parser.add_argument('-w', '--wingman-loss-weight', type=float, help='weight for wingman loss', metavar='W', default=1)
...
def main():
    ...
    args.save_path = 'checkpoints'/save_path/str(timestamp)
    ...
    disp_net = torch.nn.DataParallel(disp_net).to(device)
    pose_net = torch.nn.DataParallel(pose_net).to(device)
    ...
    # write full log to csv file
    with open(args.save_path/args.log_full, 'w') as csvfile:
        writer = csv.writer(csvfile, delimiter='\t')
        writer.writerow(['train_loss', 'photo_loss', 'smooth_loss', 'geometry_consistency_loss', 'wingman_loss'])
...
def train(args, train_loader, disp_net, pose_net, optimizer, epoch_size, logger, train_writer):
    ...
    w1, w2, w3, w4 = args.photo_loss_weight, args.smooth_loss_weight, args.geometry_consistency_weight, args.wingman_loss_weight
    ...
    for i, (tgt_img, ref_imgs, intrinsics, intrinsics_inv) in enumerate(train_loader):
        ...
        # split image to left and right image for computing wing_loss
        blank_img = torch.zeros(args.batch_size, 3, 256, 416)
        tgt_img_left = torch.cat((tgt_img[:, :, :, 0:416], blank_img), 3)
        tgt_img_right = torch.cat((tgt_img[:, :, :, 416:832], blank_img), 3)

        ref_imgs_left = [torch.cat((img[:, :, :, 0:416], blank_img), 3) for img in ref_imgs] # include: previous image, recent, and continous
        ref_imgs_right = [torch.cat((img[:, :, :, 416:832], blank_img), 3) for img in ref_imgs] # include: previous image, recent, and continous

        tgt_img = tgt_img.to(device) # recent image
        ref_imgs = [img.to(device) for img in ref_imgs] # include: previous image, recent, and continous
        intrinsics = intrinsics.to(device)

        # compute output    
        tgt_depth, ref_depths = compute_depth(disp_net, tgt_img, ref_imgs)
        poses, poses_inv, poses_left, poses_right = compute_pose_with_inv(pose_net, tgt_img, ref_imgs, tgt_img_left, ref_imgs_left, tgt_img_right, ref_imgs_right)
        ...
        loss_4 = compute_wingman_loss(poses_left, poses_right)
        ...
        if log_losses:
            ...
            train_writer.add_scalar('wingman_loss', loss_4.item(), n_iter)
...
def validate_without_gt(args, val_loader, disp_net, pose_net, epoch, logger, output_writers=[]):
    global device
    batch_time = AverageMeter()
    losses = AverageMeter(i=5, precision=5)
    ...
    return losses.avg, ['Total loss', 'Photo loss', 'Smooth loss', 'Consistency loss', 'wingman_loss']
...
def compute_pose_with_inv(pose_net, tgt_img, ref_imgs, tgt_img_left, ref_imgs_left, tgt_img_right, ref_imgs_right):
    poses_left = []
    poses_right = []
    poses = []
    poses_inv = []
    for i in range(len(ref_imgs)):
        poses.append(pose_net(tgt_img, ref_imgs[i]))
        poses_left.append(pose_net(tgt_img_left, ref_imgs_left[i]))
        poses_right.append(pose_net(tgt_img_right, ref_imgs_right[i]))
        poses_inv.append(pose_net(ref_imgs[i], tgt_img))

    return poses, poses_inv, poses_left, poses_right
